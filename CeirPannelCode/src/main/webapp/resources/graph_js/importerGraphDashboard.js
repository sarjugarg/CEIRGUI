...
$(document).ready(function(){
	...
	//alert(reportnameId);
	if(reportnameId==37 || reportnameId==36 || reportnameId==45){
	graphRequest={
	...
	else if(reportnameId==50 || reportnameId==31){
		graphRequest={
...
//boxes
	...
	var header = $("meta[name='_csrf_header']").attr("content");
	var graphRequest1="";
	[37,41,36,44].forEach(function(reportnameId) {
	$.ajaxSetup({
	headers:
	{ 'X-CSRF-TOKEN': token }
	});
...
var key1='monthWise.';
				Object.keys(data['rowData'][0]).map(function(key){ 
					...
					if(data['rowData'][0][key]!=null )
				{
						if(reportnameId==41 || reportnameId==44){
	...
	function MonthlyStockGraph(response,id,chartType,chartTitle)
	{
